...
START:
    ...
    ; cargar la tabla de valores para los números 0-F en el display de 7 
...
MAIN_LOOP:
    RCALL   T100ms  ; esperar 100 ms antes de actualizar
    INC     R17     ; incrementar el contador de tiempo
    ANDI    R17, 0x0F  
    
    IN      R16, PORTC   ; leer el estado actual de PORTC
    ANDI    R16, 0x10    
    MOV     R22, R17     ; copiar el contador a R22
    ANDI    R22, 0x0F    
    OR      R16, R22     ; combinar con el estado del LED
...
ccon:
    ...
    RJMP    alrm0      ; si está encendid apagar

alrm1:
    SBI     PORTC, 5  ; encender LED en PC4
    LDI     R16, 1
    MOV     R21, R16 ; marcar LED como encendido
    RET
    
alrm0:
    CBI     PORTC, 5  ; apagar LED en PC4
